...
3. Sections (thread each output into the next), in CLAUDE.md order:
   `scope-and-topology` → `tool-spec` → `memory-spec` → `eval-plan` → `hitl-and-autonomy` →
   `safety-and-guardrails` → `reliability-and-failure` → `model-and-cost` →
   `observability-and-ops` → `learning-loop`. Use whichever section skills are installed;
